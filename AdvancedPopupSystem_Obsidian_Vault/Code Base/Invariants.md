...
- **One instance per unique (Lane-A) popup — every Lane-A load goes through `SharedLoadAsync`.** `TryGetPopup`/`IsLive`
  see only **finished** loads (a popup registers itself from its `Awake`), so they cannot dedup a load still in flight:
  a site that calls `Resolver.LoadAsync` directly duplicates any popup a preload or a `Show<T>` is already loading — two
  instances in `AllPopups`, a `PopupCacheByType` pointing at the wrong one, a leaked handle. `SpawnAsync` (Lane B) is the
  single deliberate exception; it exists to make copies. See [[Addressables]] "Gotchas".
